...
use pevm::{
    Bytecodes, ChainState, EvmAccount, EvmCode, InMemoryStorage, Pexe, PexeError,
    PexeTxExecutionResult,
};
    ...
    unit.post.into_par_iter().for_each(|(spec_name, tests)| {
        ...
        tests.into_par_iter().for_each(|test| {
            ...
            match (
                ...
                Pexe::default().execute_revm_parallel(
                ...
                (Some("TR_NonceHasMaxValue"), Err(err)) => {
                    assert!(matches!(err, PexeError::NonceMismatch { .. }))
                }
                ...
                (Some(exception), Err(PexeError::ExecutionError(error))) => {
                ...
                (None, Ok(exec_results)) => {
                    ...
                    let PexeTxExecutionResult {receipt, state} = exec_results[0].clone();
