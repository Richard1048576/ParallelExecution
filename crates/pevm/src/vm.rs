...
pub struct PexeTxExecutionResult {
...
pub(crate) struct VmExecutionResult {
    pub(crate) execution_result: PexeTxExecutionResult,
...
struct VmDb<'a, S: Storage, C: PexeChain> {
    vm: &'a Vm<'a, S, C>,
    tx_idx: TxIdx,
    tx: &'a TxEnv,
...
impl<'a, S: Storage, C: PexeChain> VmDb<'a, S, C> {
    fn new(
        vm: &'a Vm<'a, S, C>,
        tx_idx: TxIdx,
        tx: &'a TxEnv,
...
impl<S: Storage, C: PexeChain> Database for VmDb<'_, S, C> {
...
pub(crate) struct Vm<'a, S: Storage, C: PexeChain> {
    storage: &'a S,
...
impl<'a, S: Storage, C: PexeChain> Vm<'a, S, C> {
    pub(crate) fn new(
        storage: &'a S,
        mv_memory: &'a MvMemory,
        chain: &'a C,
        ...
        match evm.transact() {
            Ok(result_and_state) => {
                ...
                Ok(VmExecutionResult {
                    execution_result: PexeTxExecutionResult::from_revm(
                        self.chain,
                        self.spec_id,
                        result_and_state,
                    ),
...
pub(crate) fn build_evm<'a, DB: Database, C: PexeChain>(
    db: DB,
    chain: &C,
    spec_id: SpecId,
    block_env: BlockEnv,
