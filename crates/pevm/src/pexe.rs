...
use crate::{
    ...
    vm::{
        ExecutionError, PexeTxExecutionResult, Vm, VmExecutionError, VmExecutionResult, build_evm,
    },
};


/// Errors when executing a block with pexe.
// TODO: implement traits explicitly due to trait bounds on `C` instead of types of `PexeChain`
#[derive(Debug, Clone, PartialEq, thiserror::Error)]
pub enum PexeError<C: PexeChain> {
    ...
    #[error(
        "PEVM encountered a bug. Please open an issue in https://github.com/Richard1048576/ParallelExecution/issues/new"
    )]
    UnreachableError,
}

/// Execution result of a block
pub type PexeResult<C> = Result<Vec<PexeTxExecutionResult>, PexeError<C>>;
...
/// The main pexe struct that executes blocks.
pub struct Pexe {
    execution_results: Vec<Mutex<Option<PexeTxExecutionResult>>>,
...
impl Pexe {
    ...
    where
        C: PexeChain + Send + Sync,
        S: Storage + Send + Sync,
    {
        let spec_id = chain
            .get_block_spec(&block.header)
            .map_err(PexeError::BlockSpecError)?;
        let block_env = get_block_env(&block.header, spec_id);
        // TODO(ZZ): refactor here with new TxEnv
        let tx_envs = match &block.transactions {
            BlockTransactions::Full(txs) => txs
                .iter()
                .map(|tx| chain.get_tx_env(tx))
                .collect::<Result<Vec<TxEnv>, _>>()
                .map_err(PexeError::InvalidTransaction)?,
            _ => return Err(PexeError::MissingTransactionData),
        };
        // TODO(ZZ): Continue to fine tune this condition.
        if force_sequential
        ...
        if let Some(abort_reason) = self.abort_reason.take() {
            match abort_reason {
                ...
                AbortReason::ExecutionError(err) => {
                    ...
                    return Err(PexeError::ExecutionError(err));
                }
            }
        }
        ...
        for address in mv_memory.consume_lazy_addresses() {
            ...
            if let Some(write_history) = mv_memory.data.get(&location_hash) {
                ...
                let code_hash = match storage.code_hash(&address) {
                    ...
                    Err(err) => return Err(PexeError::StorageError(err.to_string())),
                };
                ...
                for (tx_idx, memory_entry) in write_history.iter() {
                    ...
                    // TODO: Deduplicate this logic with [PexeTxExecutionResult::from_revm]
    ...
    fn try_execute<S: Storage, C: PexeChain>(
        &self,
        vm: &Vm<'_, S, C>,
        scheduler: &Scheduler,
...
pub fn execute_revm_sequential<S: Storage, C: PexeChain>(
    chain: &C,
    storage: &S,
    spec_id: SpecId,
    block_env: BlockEnv,
    ...
    for tx in txs {
        *evm.tx_mut() = tx;
        ...
        let mut execution_result =
            PexeTxExecutionResult::from_revm(chain, spec_id, result_and_state);
